...
export function useWelcomeAppleNotes({ gw, loadConfig, setError, setStatus }: UseWelcomeAppleNotesInput) {
  const enableAppleNotes = React.useCallback(async (params?: { memoResolvedPath?: string | null }): Promise<boolean> => {
    setError(null);
    ...
    try {
      ...
      const memoResolvedPath =
        typeof params?.memoResolvedPath === "string" && params.memoResolvedPath.trim()
          ...
          : null;
      const patterns = memoResolvedPath ? ["**/memo", "**/which", memoResolvedPath] : ["**/memo", "**/which"];
